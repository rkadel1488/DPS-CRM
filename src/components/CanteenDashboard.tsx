...
export default function CanteenDashboard({
  ...
  const [activeTab, setActiveTab] = useState<
    "menu" | "billing" | "inventory" | "planner" | "attendance"
  >("menu");
  ...
  useEffect(() => {
    ...
    const unsubscribePlans = onSnapshot(
      ...
      (snapshot) => {
        setMealPlans(
          snapshot.docs.map(
            (doc) => ({ id: doc.id, ...doc.data() }) as MealPlan,
          ),
        );
      },
      (error) => {
    ...
    const unsubscribeTeachers = onSnapshot(
      ...
      (snapshot) => {
        const allUsers = snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as unknown as UserProfile,
        );
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">
            Manage menus, billing, and meal planning.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 space-y-6">
              ...
              {meals.length > 0 ? (
                meals.map((meal) => (
                  <motion.div
                    key={meal.id}
                    layout
                    className="bg-white/70 backdrop-blur-xl p-5 rounded-2xl md:rounded-[1.5rem] border border-white/60 shadow-2xl shadow-gray-200/50 hover:shadow-md transition-all group"
                  >
              ...
              <div className="divide-y divide-gray-100/80">
                {days.map((day) => {
                  ...
                  return (
                    <div
                      key={day}
                      className="p-6 flex items-center justify-between hover:bg-white/60 backdrop-blur-md transition-all"
                    >
                      <div className="flex items-center gap-6">
                        <div className="w-16 text-sm font-bold text-gray-400 uppercase tracking-wider">
                          {day.slice(0, 3)}
                        </div>
                        <div>
                          <h4 className="font-bold text-gray-900">
                            {plan ? plan.mealName : "No meal planned"}
                          </h4>
                          <p className="text-xs text-gray-500">
                            {plan ? "Balanced Nutrition" : "Click to set meal"}
                          </p>
                        </div>
                      </div>
          ...
          {activeTab === "attendance" && (
            <div className="bg-white/70 backdrop-blur-xl rounded-3xl md:rounded-[2rem] border border-white/60 shadow-2xl shadow-gray-200/50 overflow-hidden flex flex-col h-[600px]">
              <div className="p-6 border-b border-white/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h3 className="font-bold text-gray-900">
                    Daily Lunch Attendance
                  </h3>
                  <p className="text-sm text-gray-500">
                    Mark present for lunch. Data auto-deletes after 45 days.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {attendanceType === "students"
                    ...
                    : teachers.map((teacher) => {
                        const isPresent =
                          attendanceRecord[teacher.uid] || false;
                        return (
                          <div
                            key={teacher.uid}
                  ...
                  {attendanceType === "students" && students.length === 0 && (
                    <div className="col-span-full py-10 text-center text-gray-500">
                      No students found. Add students in the Management tab
                      first.
                    </div>
                  )}
                  {attendanceType === "teachers" && teachers.length === 0 && (
                    <div className="col-span-full py-10 text-center text-gray-500">
                      No teachers found. Add teachers in the Management tab
                      first.
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-9 pr-4 py-1.5 bg-white/60 backdrop-blur-md border-none rounded-lg text-sm outline-none"
                    />
                  </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left min-w-[600px]">
                  <thead className="bg-white/60 backdrop-blur-md text-gray-400 text-xs uppercase tracking-wider">
                    <tr>
                      <th className="px-4 py-3 md:px-6 md:py-4 font-bold">
                        Student Name
                      </th>
                      <th className="px-4 py-3 md:px-6 md:py-4 font-bold">
                        Type
                  ...
                  <tbody className="divide-y divide-gray-100/80">
                    {transactions.map((t) => {
                      ...
                      return (
                        <tr
                          key={t.id}
                          className="hover:bg-white/60 backdrop-blur-md transition-all cursor-pointer"
                        >
                          <td className="px-4 py-3 md:px-6 md:py-4 font-medium text-gray-900">
                            {student ? student.name : "Unknown Student"}
                          </td>
                          <td className="px-4 py-3 md:px-6 md:py-4 capitalize text-sm text-gray-500">
                            {t.type}
                          </td>
                          <td
                            className={`px-4 py-3 md:px-6 md:py-4 font-bold ${t.type === "credit" ? "text-emerald-600" : "text-red-600"}`}
                          >
                            {t.type === "credit" ? "+" : "-"}$
                            {t.amount.toFixed(2)}
                          </td>
            ...
            <form onSubmit={handleAddMeal} className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setNewMeal({ ...newMeal, available: e.target.checked })
                  }
