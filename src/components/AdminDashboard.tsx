...
export default function AdminDashboard({
  ...
  const handleImageUploadFamily = async (
    e: React.ChangeEvent<HTMLInputElement>,
    type: "father" | "mother" | "driver" | "other" | "student",
    studentIndex?: number,
  ) => {
  ...
  const downloadGroupQRCode = async (
    family: typeof newFamily,
    familyId: string,
  ) => {
    try {
      const canvas = document.createElement("canvas");
      ...
      const items = [
        {
          ...
          photo: family.fatherPhotoUrl,
        },
        {
          label: "MOTHER",
          name: family.motherName,
          photo: family.motherPhotoUrl,
        },
        {
          label: "DRIVER",
          name: family.driverName,
          photo: family.driverPhotoUrl,
        },
  ...
  const downloadQRCode = async (student: Student) => {
    if (student.familyId) {
      const familyStudents = students.filter(
        (s) => s.familyId === student.familyId,
      );
      const reconstructedFamily = {
        fatherName: student.fatherName || "",
        fatherPhotoUrl: student.fatherPhotoUrl || "",
        motherName: student.motherName || "",
        motherPhotoUrl: student.motherPhotoUrl || "",
        driverName: student.driverName || "",
        driverPhotoUrl: student.driverPhotoUrl || "",
        otherName: student.otherName || "",
        otherPhotoUrl: student.otherPhotoUrl || "",
    ...
    try {
      ...
      const qrData =
        student.familyId || student.id || student.studentId || student.name;
      const appUrl =
        typeof window !== "undefined" ? window.location.origin : "";
      const qrDataUrl = await QRCode.toDataURL(`${appUrl}/?verify=${qrData}`, {
        width: 400,
        margin: 1,
      });
      const qrImg = await loadImage(qrDataUrl);
      ...
      const items = [
        {
          ...
          photo: student.fatherPhotoUrl,
        },
        {
          label: "MOTHER",
          name: student.motherName,
          photo: student.motherPhotoUrl,
        },
        {
          label: "DRIVER",
          name: student.driverName,
          photo: student.driverPhotoUrl,
        },
        {
          label: "OTHER",
          name: student.otherName,
          photo: student.otherPhotoUrl,
        },
      ];
  ...
  const handleImageUpload = async (
    e: React.ChangeEvent<HTMLInputElement>,
    isEdit: boolean,
    type: "student" | "father" | "mother" | "driver" | "other",
  ) => {
    const file = e.target.files?.[0];
    if (!file) return;
  ...
  const handleAddStaff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addAppNotification(
        "Staff Invited",
        `${newStaff.name} was invited as ${newStaff.role}.`,
        "info",
      );
  ...
  useEffect(() => {
    ...
    const unsubscribeInvites = onSnapshot(
      ...
      (snapshot) => {
        setInvites(
          snapshot.docs.map(
            (doc) => ({ id: doc.id, ...doc.data() }) as StaffInvite,
          ),
        );
      },
      (error) => {
  ...
  const staffMembers = staff.filter(
    (s) => s.role !== "parent" && s.role !== "teacher",
  );

  const pendingTeacherInvites = pendingInvites.filter(
    (i) => i.role === "teacher",
  );
  const pendingParentInvites = pendingInvites.filter(
    (i) => i.role === "parent",
  );
  const pendingStaffInvites = pendingInvites.filter(
    (i) => i.role !== "parent" && i.role !== "teacher",
  );
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">
            Manage student records, staff roles, and parent access.
          </p>
        </div>
                ...
                <PieChart>
                  <Pie
                    data={[
                      ...
                      {
                        name: "Teachers",
                        value: staff.filter((s) => s.role === "teacher").length,
                      },
                      {
                        name: "Parents",
                        value: staff.filter((s) => s.role === "parent").length,
                      },
                      {
                        name: "Staff",
        ...
        <div className="overflow-x-auto">
            ...
            <table className="w-full text-left min-w-[600px]">
              <thead className="bg-white/60 backdrop-blur-md text-gray-400 text-[10px] uppercase tracking-widest font-bold">
                <tr>
                  ...
                  <th className="px-4 py-3 md:px-6 md:py-4">Route</th>
                  <th className="px-4 py-3 md:px-6 md:py-4">Balance</th>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 md:px-6 md:py-4">
                        <span className="text-sm font-medium text-gray-700">
                          {student.routeId
                            ? routes.find((r) => r.id === student.routeId)
                                ?.name || "Unknown Route"
                            : "Unassigned"}
                        </span>
                      </td>
                      <td className="px-4 py-3 md:px-6 md:py-4">
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-2 h-2 rounded-full ${student.balance > 0 ? "bg-emerald-500" : "bg-red-500"}`}
                          ></div>
                    ...
                    <td colSpan={5} className="px-6 py-20 text-center">
                      <div className="max-w-xs mx-auto">
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                          Start by adding students or staff members to the
                          system.
                        </p>
                      </div>
                    </td>
          ...
          {activeTab === "teachers" && (
            <table className="w-full text-left min-w-[600px]">
              <thead className="bg-white/60 backdrop-blur-md text-gray-400 text-[10px] uppercase tracking-widest font-bold">
                <tr>
                  <th className="px-4 py-3 md:px-6 md:py-4">Teacher Name</th>
                  <th className="px-4 py-3 md:px-6 md:py-4">Contact</th>
              ...
              <tbody className="divide-y divide-gray-100/80">
                {teacherMembers.map((member) => (
                  <tr
                    key={member.uid}
                    className="hover:bg-white/60 backdrop-blur-md transition-all"
                  >
                    <td className="px-4 py-3 md:px-6 md:py-4">
                      <div className="flex items-center gap-3">
                        <img
                    ...
                    <td className="px-4 py-3 md:px-6 md:py-4">
                      <select
                        value={member.role}
                        onChange={(e) =>
                          updateRole(member, e.target.value as UserRole)
                        }
                ...
                {pendingTeacherInvites.map((invite) => (
                  <tr
                    key={invite.id}
                    className="hover:bg-white/60 backdrop-blur-md transition-all opacity-60"
                  >
                    <td className="px-4 py-3 md:px-6 md:py-4">
                      <div className="flex items-center gap-3">
                        <img
          ...
          {activeTab === "staff" && (
            <table className="w-full text-left min-w-[600px]">
              <thead className="bg-white/60 backdrop-blur-md text-gray-400 text-[10px] uppercase tracking-widest font-bold">
                <tr>
                  <th className="px-4 py-3 md:px-6 md:py-4">Staff Member</th>
                  <th className="px-4 py-3 md:px-6 md:py-4">Role</th>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {availableTabs.map((tab) => {
                          ...
                          const hasAccess =
                            isMainAdmin ||
                            member.role === "admin" ||
                            member.allowedTabs?.includes(tab.id);
                          return (
                            <button
                              key={tab.id}
                              disabled={isMainAdmin || member.role === "admin"}
                              onClick={() =>
                                toggleTabPermission(member, tab.id)
                              }
                    ...
                    <td className="px-4 py-3 md:px-6 md:py-4">
                      <div className="flex flex-wrap gap-2">
                        {availableTabs.map((tab) => {
                          const hasAccess =
                            invite.role === "admin" ||
                            invite.allowedTabs?.includes(tab.id);
                          return (
                            <div
                              key={tab.id}
                              className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold transition-all ${
          ...
          {activeTab === "parents" && (
            <table className="w-full text-left min-w-[600px]">
              <thead className="bg-white/60 backdrop-blur-md text-gray-400 text-[10px] uppercase tracking-widest font-bold">
                <tr>
                  <th className="px-4 py-3 md:px-6 md:py-4">Parent Name</th>
                  <th className="px-4 py-3 md:px-6 md:py-4">Contact</th>
                  <th className="px-4 py-3 md:px-6 md:py-4">Role</th>
                  <th className="px-4 py-3 md:px-6 md:py-4 text-right">
                    Actions
                    ...
                    <tr>
                      <td colSpan={4} className="px-6 py-20 text-center">
                        <div className="max-w-xs mx-auto">
                          ...
                          <p className="text-xs text-gray-400 mt-1">
                            Start by adding parents to the system.
                          </p>
                        </div>
                      </td>
                    </tr>
      ...
      {studentToDelete && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div
            ...
            className="bg-white/70 backdrop-blur-xl rounded-3xl md:rounded-[2rem] p-6 w-full max-w-sm shadow-2xl shadow-gray-200/40 text-center"
          >
            <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <Trash2 className="w-6 h-6 text-red-600" />
            </div>
            <h3 className="text-[1.35rem] font-extrabold tracking-tight text-gray-900 mb-2">
              Delete Student
            </h3>
      ...
      {inviteToDelete && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <p className="text-gray-500 mb-6">
              Are you sure you want to delete this pending invite? This action
              cannot be undone.
            </p>
            <div className="flex gap-3">
              <button
      ...
      {isAddingStaff && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="bg-white/70 backdrop-blur-xl rounded-3xl md:rounded-[2rem] p-6 w-full max-w-md shadow-2xl shadow-gray-200/40"
          >
            <h3 className="text-[1.35rem] font-extrabold tracking-tight text-gray-900 mb-4">
              Add New User
            </h3>
            ...
            >
              <div className="flex flex-col items-center gap-4 py-4">
                <div className="relative group">
                  <div className="w-24 h-16 md:h-24 rounded-[1rem] bg-gray-100 border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden">
                  ...
                  <label className="absolute -bottom-2 -right-2 p-2 bg-gradient-to-r from-emerald-500 to-teal-500 shadow-lg shadow-emerald-500/20 border-none text-white rounded-xl shadow-lg cursor-pointer hover:from-emerald-600 hover:to-teal-600 hover:shadow-xl hover:-translate-y-0.5 transition-all">
                    ...
                    <input
                      ...
                      accept="image/*"
                      className="hidden"
                      onChange={(e) => handleImageUpload(e, true, "student")}
                    />
                  </label>
                </div>
                  ...
                  <div className="relative">
                    <div className="w-16 h-16 rounded-xl bg-white border border-white/60 flex items-center justify-center overflow-hidden">
                      {editingStudent.fatherPhotoUrl ? (
                        <img
                          src={editingStudent.fatherPhotoUrl}
                          alt="Father"
                          className="w-full h-full object-cover"
                        />
                      ) : (
                    ...
                    <label className="absolute -bottom-1 -right-1 p-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg shadow-2xl shadow-gray-200/50 cursor-pointer hover:bg-black transition-all">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleImageUpload(e, true, "father")}
                      />
                    </label>
                  </div>
                  ...
                  <div className="relative">
                    <div className="w-16 h-16 rounded-xl bg-white border border-white/60 flex items-center justify-center overflow-hidden">
                      {editingStudent.motherPhotoUrl ? (
                        <img
                          src={editingStudent.motherPhotoUrl}
                          alt="Mother"
                          className="w-full h-full object-cover"
                        />
                      ) : (
                    ...
                    <label className="absolute -bottom-1 -right-1 p-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg shadow-2xl shadow-gray-200/50 cursor-pointer hover:bg-black transition-all">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleImageUpload(e, true, "mother")}
                      />
                    </label>
                  </div>
                  <div className="flex-1">
                  ...
                  <div className="relative">
                    <div className="w-16 h-16 rounded-xl bg-white border border-white/60 flex items-center justify-center overflow-hidden">
                      {editingStudent.driverPhotoUrl ? (
                        <img
                          src={editingStudent.driverPhotoUrl}
                          alt="Driver"
                          className="w-full h-full object-cover"
                        />
                      ) : (
                    ...
                    <label className="absolute -bottom-1 -right-1 p-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg shadow-2xl shadow-gray-200/50 cursor-pointer hover:bg-black transition-all">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleImageUpload(e, true, "driver")}
                      />
                    </label>
                  </div>
                  <div className="flex-1">
                  ...
                  <div className="relative">
                    <div className="w-16 h-16 rounded-xl bg-white border border-white/60 flex items-center justify-center overflow-hidden">
                      {editingStudent.otherPhotoUrl ? (
                        <img
                          src={editingStudent.otherPhotoUrl}
                          alt="Other"
                          className="w-full h-full object-cover"
                        />
                      ) : (
                    ...
                    <label className="absolute -bottom-1 -right-1 p-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg shadow-2xl shadow-gray-200/50 cursor-pointer hover:bg-black transition-all">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleImageUpload(e, true, "other")}
                      />
                    </label>
                  </div>
                  <div className="flex-1">
      ...
      {isAddingFamily && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="bg-white/70 backdrop-blur-xl rounded-3xl md:rounded-[2rem] w-full max-w-4xl shadow-2xl shadow-gray-200/40 overflow-hidden flex flex-col max-h-[90vh]"
          >
            <div className="p-6 border-b border-white/60 shrink-0 flex items-center justify-between">
              <div>
                <h3 className="text-[1.35rem] font-extrabold tracking-tight text-gray-900">
                  Add Group Gate Pass
                </h3>
                <p className="text-xs text-gray-500">
                  Create a shared gate pass for multiple students
                </p>
              </div>
              <button
            ...
            >
              <div className="space-y-4">
                <h4 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                  <Users className="w-4 h-4" /> 1. Family & Pickup Details
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-col items-center gap-3 p-4 bg-white/60 backdrop-blur-md rounded-[1rem] border border-white/60">
                      <div className="w-20 h-20 rounded-xl bg-white border border-white/60 flex items-center justify-center overflow-hidden">
                        {newFamily.fatherPhotoUrl ? (
                          <img
                            src={newFamily.fatherPhotoUrl}
                            className="w-full h-full object-cover"
                          />
                        ) : (
                      ...
                      <label className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg text-[10px] uppercase font-bold cursor-pointer hover:bg-black transition-all">
                        ...
                        <input
                          ...
                          accept="image/*"
                          className="hidden"
                          onChange={(e) => handleImageUploadFamily(e, "father")}
                        />
                      </label>
                      <div className="w-full mt-2">
                        ...
                        <input
                          ...
                          className="w-full px-3 py-1.5 bg-white border border-black/10 rounded-lg text-sm text-center focus:ring-2 focus:ring-blue-500 outline-none"
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-col items-center gap-3 p-4 bg-white/60 backdrop-blur-md rounded-[1rem] border border-white/60">
                      <div className="w-20 h-20 rounded-xl bg-white border border-white/60 flex items-center justify-center overflow-hidden">
                        {newFamily.motherPhotoUrl ? (
                          <img
                            src={newFamily.motherPhotoUrl}
                            className="w-full h-full object-cover"
                          />
                        ) : (
                          <Camera className="w-8 h-8 text-gray-300" />
                      ...
                      <label className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg text-[10px] uppercase font-bold cursor-pointer hover:bg-black transition-all">
                        ...
                        <input
                          ...
                          accept="image/*"
                          className="hidden"
                          onChange={(e) => handleImageUploadFamily(e, "mother")}
                        />
                      </label>
                      <div className="w-full mt-2">
                        <label className="block text-xs font-medium text-gray-500 mb-1 text-center">
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-col items-center gap-3 p-4 bg-white/60 backdrop-blur-md rounded-[1rem] border border-white/60">
                      <div className="w-20 h-20 rounded-xl bg-white border border-white/60 flex items-center justify-center overflow-hidden">
                        {newFamily.driverPhotoUrl ? (
                          <img
                            src={newFamily.driverPhotoUrl}
                            className="w-full h-full object-cover"
                          />
                        ) : (
                          <Camera className="w-8 h-8 text-gray-300" />
                      ...
                      <label className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg text-[10px] uppercase font-bold cursor-pointer hover:bg-black transition-all">
                        ...
                        <input
                          ...
                          accept="image/*"
                          className="hidden"
                          onChange={(e) => handleImageUploadFamily(e, "driver")}
                        />
                      </label>
                      <div className="w-full mt-2">
                        <label className="block text-xs font-medium text-gray-500 mb-1 text-center">
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-col items-center gap-3 p-4 bg-white/60 backdrop-blur-md rounded-[1rem] border border-white/60">
                      <div className="w-20 h-20 rounded-xl bg-white border border-white/60 flex items-center justify-center overflow-hidden">
                        {newFamily.otherPhotoUrl ? (
                          <img
                            src={newFamily.otherPhotoUrl}
                            className="w-full h-full object-cover"
                          />
                        ) : (
                          <Camera className="w-8 h-8 text-gray-300" />
                      ...
                      <label className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg text-[10px] uppercase font-bold cursor-pointer hover:bg-black transition-all">
                        ...
                        <input
                          ...
                          accept="image/*"
                          className="hidden"
                          onChange={(e) => handleImageUploadFamily(e, "other")}
                        />
                      </label>
                      <div className="w-full mt-2">
                        <label className="block text-xs font-medium text-gray-500 mb-1 text-center">
                ...
                <div className="w-full max-w-sm">
                  <label className="block text-xs font-medium text-gray-500 mb-1">
                    Primary Contact Number (For WhatsApp Alerts)
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full px-4 py-2 bg-white/60 backdrop-blur-md border border-black/10 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none"
                    placeholder="+1234567890"
                  />
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                    <GraduationCap className="w-4 h-4" /> 2. Students in Group
                  </h4>
                  <button
                    type="button"
                    onClick={() =>
                ...
                <div className="space-y-4">
                  {newFamily.students.map((student, index) => (
                    <div
                      key={index}
                      className="flex flex-col lg:flex-row items-center lg:items-start gap-4 p-4 rounded-[1rem] border border-white/60 bg-white/60 backdrop-blur-md/50"
                    >
                      <div className="flex flex-col items-center gap-2 shrink-0">
                        ...
                        <label className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg text-[10px] uppercase font-bold cursor-pointer hover:bg-black transition-all">
                          ...
                          <input
                            ...
                            className="hidden"
                            onChange={(e) =>
                              handleImageUploadFamily(e, "student", index)
                            }
                          />
                        </label>
                      </div>
                      ...
                      <div className="flex-1 grid grid-cols-2 lg:grid-cols-4 gap-4 w-full">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const s = [...newFamily.students];
                              s[index].studentId = e.target.value;
                              setNewFamily({ ...newFamily, students: s });
                            }}
                            className="w-full px-3 py-2 text-sm bg-white border border-black/10 rounded-lg outline-none focus:border-blue-500"
                          />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const s = [...newFamily.students];
                              s[index].section = e.target.value;
                              setNewFamily({ ...newFamily, students: s });
                            }}
                            className="w-full px-3 py-2 text-sm bg-white border border-black/10 rounded-lg outline-none focus:border-blue-500"
                            placeholder="e.g. A"
                      ...
                      {newFamily.students.length > 1 && (
                        <button
                          ...
                          className="p-2 text-red-400 hover:bg-red-50 rounded-lg shrink-0 mt-4 lg:mt-0"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      )}
              ...
              <div className="pt-4 shrink-0 flex gap-3">
                ...
                <button
                  type="submit"
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 shadow-lg shadow-blue-500/20 border-none text-white rounded-xl font-bold hover:from-blue-600 hover:to-indigo-600 hover:shadow-xl hover:-translate-y-0.5 transition-all"
                >
            ...
            >
                ...
                <div className="relative group">
                  ...
                  <label className="absolute -bottom-2 -right-2 p-2 bg-gradient-to-r from-emerald-500 to-teal-500 shadow-lg shadow-emerald-500/20 border-none text-white rounded-xl shadow-lg cursor-pointer hover:from-emerald-600 hover:to-teal-600 hover:shadow-xl hover:-translate-y-0.5 transition-all">
                    ...
                    <input
                      ...
                      accept="image/*"
                      className="hidden"
                      onChange={(e) => handleImageUpload(e, false, "student")}
                    />
                  </label>
                </div>
                <p className="text-[10px] text-gray-400 font-bold uppercase">
                  ...
                  <div className="relative">
                    ...
                    <label className="absolute -bottom-1 -right-1 p-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg shadow-2xl shadow-gray-200/50 cursor-pointer hover:bg-black transition-all">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleImageUpload(e, false, "father")}
                      />
                    </label>
                  </div>
                  <div className="flex-1">
                  ...
                  <div className="relative">
                    ...
                    <label className="absolute -bottom-1 -right-1 p-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg shadow-2xl shadow-gray-200/50 cursor-pointer hover:bg-black transition-all">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleImageUpload(e, false, "mother")}
                      />
                    </label>
                  </div>
                  <div className="flex-1">
                  ...
                  <div className="relative">
                    ...
                    <label className="absolute -bottom-1 -right-1 p-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg shadow-2xl shadow-gray-200/50 cursor-pointer hover:bg-black transition-all">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleImageUpload(e, false, "driver")}
                      />
                    </label>
                  </div>
                  <div className="flex-1">
                  ...
                  <div className="relative">
                    ...
                    <label className="absolute -bottom-1 -right-1 p-1.5 bg-gradient-to-r from-slate-700 to-slate-900 shadow-lg shadow-slate-900/20 text-white border-none text-white rounded-lg shadow-2xl shadow-gray-200/50 cursor-pointer hover:bg-black transition-all">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleImageUpload(e, false, "other")}
                      />
                    </label>
                  </div>
                  <div className="flex-1">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewStudent({ ...newStudent, routeId: e.target.value })
                  }
                  className="w-full px-4 py-2 bg-white/60 backdrop-blur-md border-none rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none"
                >
                  <option value="">No Route Assigned</option>
