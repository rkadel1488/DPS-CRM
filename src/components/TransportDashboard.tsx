...
function MapBoundsUpdater({
  ...
  useEffect(() => {
    ...
    let points: L.LatLngExpression[] = targetVehicles.map((v) => [
      v.currentLat!,
      v.currentLng!,
    ]);

    if (selectedVehicleId !== "all") {
      const trackingRoute = routes.find(
...
export default function TransportDashboard({
  ...
  const [activeTab, setActiveTab] = useState<
    "fleet" | "routes" | "attendance" | "tracking"
  >("fleet");
  ...
  const [broadcastingVehicleId, setBroadcastingVehicleId] = useState<
    string | null
  >(null);
  const [watchId, setWatchId] = useState<number | null>(null);
  const [vehicleToDelete, setVehicleToDelete] = useState<string | null>(null);
  const [selectedTrackingVehicleId, setSelectedTrackingVehicleId] =
    useState<string>("all");
  ...
  const handleAddRoute = async (e: React.FormEvent) => {
    ...
    try {
      const mockStops = [
        { name: "Start Point", lat: 40.71, lng: -74.015 },
        { name: "Midway Station", lat: 40.715, lng: -74.005 },
        { name: "School Campus", lat: 40.72, lng: -73.995 },
      ];
      const routeData = {
        ...newRoute,
        stops: newRoute.stops.length > 0 ? newRoute.stops : mockStops,
      };
  ...
  useEffect(() => {
    ...
    const unsubscribeRoutes = onSnapshot(
      ...
      (snapshot) => {
        setRoutes(
          snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as Route),
        );
      },
      (error) => {
    ...
    const unsubscribeAttendance = onSnapshot(
      ...
      (snapshot) => {
        setAttendance(
          snapshot.docs.map(
            (doc) => ({ id: doc.id, ...doc.data() }) as TransportAttendance,
          ),
        );
      },
      (error) => {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">
            Monitor fleet, optimize routes, and track students.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 space-y-6">
              ...
              {vehicles.length > 0 ? (
                vehicles.map((bus) => (
                  <motion.div
                    key={bus.id}
                    layout
                    className="bg-white/70 backdrop-blur-xl p-5 md:p-8 rounded-3xl md:rounded-[2rem] border border-white/60 shadow-2xl shadow-gray-200/50 hover:shadow-md transition-all"
                  >
                ...
                <div className="col-span-full py-20 text-center bg-white rounded-3xl md:rounded-[2rem] border border-dashed border-gray-300">
                  <Bus className="w-12 h-12 text-gray-300 mx-auto mb-4" />
                  <p className="text-gray-500">
                    No vehicles registered. Add your first bus!
                  </p>
                  {isAdmin && (
                    <button
            ...
            <div className="space-y-4">
              {routes.length > 0 ? (
                routes.map((route) => (
                  <div
                    key={route.id}
                    className="bg-white/70 backdrop-blur-xl p-5 md:p-8 rounded-3xl md:rounded-[2rem] border border-white/60 shadow-2xl shadow-gray-200/50 flex items-center gap-6"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500">
                        ...
                        {students.filter((s) => s.routeId === route.id).length}{" "}
                        Students Assigned
                        {route.vehicleId &&
                ...
                <div className="py-20 text-center bg-white rounded-3xl md:rounded-[2rem] border border-dashed border-gray-300">
          ...
          {activeTab === "attendance" && (
            <div className="bg-white/70 backdrop-blur-xl p-5 md:p-8 rounded-3xl md:rounded-[2rem] border border-white/60 shadow-2xl shadow-gray-200/50 space-y-6">
              ...
              {selectedRouteId ? (
                <div className="space-y-4">
                  ...
                  {students.filter((s) => s.routeId === selectedRouteId)
                    .length > 0 ? (
                    students
                      ...
                      .map((student) => {
                        ...
                        return (
                          <div
                            key={student.id}
                            className="flex items-center justify-between p-4 bg-white/60 backdrop-blur-md rounded-[1rem]"
                          >
                ...
                <div className="text-center py-12 bg-white/60 backdrop-blur-md rounded-[1rem] border border-dashed border-gray-300">
                  ...
                  <p className="text-gray-500">
                    Select a route to view and mark attendance.
                  </p>
                </div>
              )}
            </div>
          ...
          {activeTab === "tracking" && (
            <div className="bg-white/70 backdrop-blur-xl rounded-3xl md:rounded-[2rem] border border-white/60 shadow-2xl shadow-gray-200/50 h-[500px] relative overflow-hidden">
              <div className="absolute inset-0 bg-gray-100">
                ...
                >
                  ...
                  <MapBoundsUpdater
                    vehicles={vehicles}
                    routes={routes}
                    selectedVehicleId={selectedTrackingVehicleId}
                  ...
                  {(() => {
                    const trackingRoute =
                      selectedTrackingVehicleId !== "all"
                        ? routes.find(
                            (r) => r.vehicleId === selectedTrackingVehicleId,
                          )
                        : null;
                    if (
                      !trackingRoute ||
                      !trackingRoute.stops ||
                      trackingRoute.stops.length === 0
                    )
                      return null;
                    return (
                      <>
                        <Polyline
                          positions={trackingRoute.stops.map((s) => [
                            s.lat,
                            s.lng,
                          ])}
                          color="#3b82f6"
                ...
                <div className="bg-white/70 backdrop-blur-xl/90 backdrop-blur p-3 rounded-[1rem] shadow-lg pointer-events-auto border border-white/60 flex flex-col gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-bold text-gray-900">
                      {
                        vehicles.filter((v) => v.currentLat && v.currentLng)
                          .length
                      }{" "}
                      Buses Online
                    </span>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSelectedTrackingVehicleId(e.target.value)
                    }
                  ...
                  <p className="text-[10px] text-gray-500">
                    Drivers can broadcast GPS from the Fleet tab.
                  </p>
                </div>
        ...
        <div className="space-y-6">
          <div className="bg-white/70 backdrop-blur-xl rounded-3xl md:rounded-[2rem] p-6 border border-white/60 shadow-2xl shadow-gray-200/50">
            ...
            <form onSubmit={handleAddRoute} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewRoute({ ...newRoute, vehicleId: e.target.value })
                  }
                  className="w-full px-4 py-2 bg-white/60 backdrop-blur-md border-none rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none"
                >
      ...
      {vehicleToDelete && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="bg-white/70 backdrop-blur-xl rounded-3xl md:rounded-[2rem] p-6 w-full max-w-md shadow-2xl shadow-gray-200/40 text-center"
          >
            ...
            <p className="text-gray-500 mb-6">
              Are you sure you want to remove this bus? This action cannot be
              undone.
            </p>
